...
if (!file_exists($filename) || serialize($result["summary"]) !== file_get_contents($filename)) {
	CleanupCache();
}
...
do {
	...
	echo "\n";
	if ($backupnum < 0 || $backupnum >= $y) {
		CB_DisplayError("Invalid backup entered.  Please try again.", false, false);
	}
...
for ($x = 0; $x <= $backupnum; $x ++) {
	...
	if (file_exists($filename)) {
		$blocklist2 = unserialize(file_get_contents($filename));
	} else {
	...
	while (count($parts)) {
		...
		if ($part === "" || $part === ".") {
			...
			if ($row === false) {
				return array(
					"success"   => false,
					"error"     => "The path/filename '" . $path . "' does not exist.",
			...
			if ($row->symlink !== "") {
				...
				if ($symlinks >= 20) {
					return array(
						"success"   => false,
						"error"     => "The specified path follows too many symbolic links.  Probably an infinite loop.",
				...
				if (count($parts)) {
					return array(
						"success"   => false,
						"error"     => "File encountered at '" . $part . "' while resolving the path '" . $path . "'.  Expected a folder or a symbolic link.",
						"errorcode" => "file_encountered",
						"info"      => $result
					);
				}
...
if ($dir !== false) {
	while (($file = readdir($dir)) !== false) {
		if (substr($file, - 4) === ".php") {
			require_once $rootpath . "/restore_shell_exts/" . $file;
		}
	}
...
while (($line = fgets(STDIN)) !== false) {
	...
	if ($cmd != "") {
		if (!function_exists("shell_cmd_" . $cmd)) {
			echo "The shell command '" . $cmd . "' does not exist.\n";
		} else {
			$cmd = "shell_cmd_" . $cmd;
			$cmd($line);
		}
