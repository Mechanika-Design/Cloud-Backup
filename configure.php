...
if (!isset($config["blocksize"])) {
	$config["blocksize"] = 10485760;
}
if (!isset($config["smallfilelimit"])) {
	$config["smallfilelimit"] = 1048576;
}
if (!isset($config["backupretryrange"])) {
	$config["backupretryrange"] = 6 * 60 * 60;
}
...
if (!isset($config["service_info"])) {
	...
	do {
		...
		if (!isset($services[$servicename])) {
			echo "Error:  Invalid service name entered.\n";
		}
	...
	do {
		...
		$result = $service->Test();
		if (!$result["success"]) {
			CB_DisplayError("The backup service access test failed.  Try entering the information again.", $result, false);
		}
	...
	do {
		...
		if (count($cmd2) == 2) {
			if (strtolower($cmd2[0]) === "add") {
				...
				if ($path !== false && $checkpaths && is_dir($path)) {
					$config[$configkey][] = $path;
				} else if ($path !== false && !$checkpaths && file_exists($path)) {
					$config[$configkey][] = $path;
				} else {
					echo "The path '" . ($path !== false ? $path : $cmd2[1]) . "' does not exist.\n";
				}
...
if (!isset($config["backup_paths"])) {
	$config["backup_paths"] = array();
}
echo "----------\n\n";
...
if (!isset($config["backup_exclusions"])) {
	$config["backup_exclusions"] = array();
}
echo "----------\n\n";
...
if (!isset($config["prebackup_commands"])) {
	$config["prebackup_commands"] = array();
}
echo "----------\n\n";
...
do {
	...
	if (strtolower($cmd2[0]) === "add") {
		...
		if (isset($config["notifications"][(int) $cmd2[1] - 1])) {
			$config["notifications"][(int) $cmd2[1] - 1]["recipients"][] = $cmd2[2];
		}
