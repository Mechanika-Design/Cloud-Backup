			...
			if (isset($args["opts"]["all"]) || substr($name, 0, 1) != ".") {
				foreach ($args["opts"]["regex"] as $pattern) {
					if (preg_match($pattern, $name)) {
						if (strlen($info["owner"]) > $maxowner) {
							$maxowner = strlen($info["owner"]);
						}
						if (strlen($info["group"]) > $maxgroup) {
							$maxgroup = strlen($info["group"]);
						}

						$fullsize = number_format($info["filesize"], 0);
						if (strlen($fullsize) > $maxfullsize) {
							$maxfullsize = strlen($fullsize);
						}
		...
		if (isset($args["opts"]["all"]) || substr($name, 0, 1) != ".") {
			...
			if ($found) {
				...
				if (isset($args["opts"]["long"])) {
					...
					if ($info["attributes"] & 0004000) {
						$attr .= "s";
					} else if ($symlink || $info["attributes"] & 0000100) {
						$attr .= "x";
					} else {
						$attr .= "-";
					}
					...
					if ($info["attributes"] & 0002000) {
						$attr .= "s";
					} else if ($symlink || $info["attributes"] & 0000010) {
						$attr .= "x";
					} else {
						$attr .= "-";
					}
					...
					if ($info["attributes"] & 0001000) {
						$attr .= "t";
					} else if ($symlink || $info["attributes"] & 0000001) {
						$attr .= "x";
					} else {
						$attr .= "-";
					}
				...
				if (isset($args["opts"]["long"]) && $info["symlink"] !== "") {
					echo " -> " . $info["symlink"];
				}
...
function shell_cmd_restore($line) {
	...
	while (count($stack)) {
		...
		if (count($stack[$pos]["files"])) {
			...
			if ($info["owner"] !== "") {
				@chown($dirfile, $info["owner"]);
			}
			if ($info["group"] !== "") {
				@chgrp($dirfile, $info["group"]);
			}
...
function shell_cmd_help($line) {
	...
	if (isset($result["user"])) {
		foreach ($result["user"] as $name) {
			if (strtolower(substr($name, 0, 10)) == "shell_cmd_") {
				$functions[] = substr($name, 10);
			}
		}
	}
