...
foreach ($config["monitor"] as $filename) {
	if (!file_exists($filename)) CB_DisplayError("The monitored '" . $filename . "' no longer exists. Backup attempt terminated.");
	if (!file_exists($filename)) CB_DisplayError("The monitored file '" . $filename . "' is not a file (probably a directory). Backup attempt terminated.");
	...
	if (!isset($config["monitor_hashes"][$filename])) {
		CB_Log("[Monitor] First hash calculation of '" . $filename . "' result in a SHA256 hash value of '" . $hash2 . "'.");
...
if (isset($args["opts"]["defrag"])) {
	...
	if ($keepsize < 0) {
		CB_DisplayError("The small file limit in the configuration prevents defragmentation.");
	}
	$lastblocknum = false;
	while ($row = $result->NextRow()) {
		...
		// There is an 8 byte ID + 4 byte size overhead in shared block entry.
...
if ($config["uploaddatalimit"] > 0 && $servicehelper->GetBytesSent() >= $config["uploaddatalimit"]) {
	CB_Log("[Warning] Upload data limit reached. Backup is incomplete.");
}
