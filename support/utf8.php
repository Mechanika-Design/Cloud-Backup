...
class UTF8 {
	// Remove invalid characters from the data string.
	...
	// Return the number of characters in UTF8 string.
	public static function strlen($data) {
		$num  = 0;
		$pos  = 0;
		$size = 0;
		$y    = strlen($data);
		while (self::NextChrPos($data, $y, $pos, $size)) {
			$num ++;
		}
	...
	// Converts a UTF8 string to ASCII and drops bad UTF8 and non-ASCII characters in teh process.
	public static function ConvertToASCII($data) {
		...
		$y    = strlen($data);
		while ($pos < $y) {
			if (self::NextChrPos($data, $y, $pos, $size) && $size == 1) {
				$result .= $data[$pos];
			} else if (!$size) {
				$size = 1;
			}
