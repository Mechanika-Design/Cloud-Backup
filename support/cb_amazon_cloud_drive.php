...
if (!class_exists("AmazonCloudDrive", false)) {
	require_once str_replace("\\", "/", dirname(__FILE__)) . "/sdk_amazon_cloud_drive.php";
}
...
class CB_Service_amazon_cloud_drive {
	...
	public function Init(&$options, $servicehelper) {
		if ($options["clientid"] === "") {
			$options["clientid"] = false;
		}
		if ($options["clientsecret"] === "") {
			$options["clientsecret"] = false;
		}
		$options["clientscope"] = array("clouddrive:read_all", "clouddrive:write");
		if ($options["returnurl"] === "") {
			$options["returnurl"] = false;
		}
		if (is_string($options["cb_embed"])) {
			$options["cb_embed"] = (strtoupper(substr(trim($options["cb_embed"]), 0, 1)) === "Y");
		}
		...
		if (isset($options["refreshtoken"])) {
			$this->acd->AddClientInfoUpdatedNotify(array($this, "ClientInfoUpdated__SaveConfig"));
		}
	...
	public function UploadBlock($blocknum, $part, $data) {
		...
		if ($this->options["cb_embed"]) {
			$this->servicehelper->ApplyPhoto($data);
		}
	...
	public function GetIncrementalBlockList($id) {
		...
		foreach ($result["files"] as $info2) {
			if (preg_match('/^(\d+)_(\d+)\.' . ($this->options["cb_embed"] ? "jpg" : "dat") . '$/', $info2["name"], $matches)) {
				...
				$blocklist[$matches[1]][$matches[2]] = array("pid"  => $id,
				                                             "id"   => $info2["id"],
				                                             "name" => $info2["name"]
				);
			}
	...
	public function DownloadBlock($info) {
		...
		if ($this->options["cb_embed"]) {
			$this->servicehelper->UnapplyPhoto($result["body"]);
		}
	...
	public function StartMergeDown() {
		...
		if ($result["info"] !== false) {
			$this->remotemergefolderid = $result["info"]["id"];
		} else {
