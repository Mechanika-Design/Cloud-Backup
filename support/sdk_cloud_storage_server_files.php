...
class CloudStorageServerFiles {
	...
	public function GetSSLInfo() {
		if ($this->host === false) {
			return array(
				"success"   => false,
				"error"     => self::CSS_Translate("Missing host."),
				"errorcode" => "no_access_info"
			);
		}

		if ($this->cafile !== false && $this->cacert === false) {
			$this->cacert = @file_get_contents($this->cafile);
		}
		...
		if ($this->cert === false) {
			return array(
				"success"   => false,
				"error"     => self::CSS_Translate("Unable to retrieve server certificate."),
				"errorcode" => "cert_retrieval_failed"
			);
		}
		if ($this->cacert === false) {
			return array(
				"success"   => false,
				"error"     => self::CSS_Translate("Unable to retrieve server CA certificate."),
				"errorcode" => "cacert_retrieval_failed"
			);
		}
	...
	public function UploadFile($folderid, $destfilename, $data, $srcfilename, $fileid = false, $callback = false, $callbackopts = false) {
		...
		if ($fileid === false) {
			...
			if (!$result["success"] && $result["errorcode"] !== "object_not_found") {
				return $result;
			}

			if ($result["success"] && $result["body"]["object"]["type"] !== "file") {
				return array(
					"success"   => false,
					"error"     => self::CSS_Translate("Parent folder already contains an object named '%s' that is not a file.", $destfilename),
	...
	public function DownloadFile($destfileorfp, $fileid, $callback = false, $database = false) {
		...
		if ($database) {
			$result = $this->RunAPI("GET", "file/downloaddatabase", $options, 200, true, false);
		} else {
			$result = $this->RunAPI("GET", "file/download/" . $fileid, $options, 200, true, false);
		}
	...
	private function RunAPI($method, $apipath, $options = array(), $expected = 200, $encodejson = true, $decodebody = true) {
		...
		if (isset($result["fp"]) && is_resource($result["fp"])) {
			$this->fp = $result["fp"];
		} else {
			$this->fp = false;
		}
		...
		if ($result["response"]["code"] == 400) {
			...
			if (is_array($error) && isset($error["success"]) && !$error["success"]) {
				return $error;
			}
		}
