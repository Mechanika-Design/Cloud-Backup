...
class SMTP {
	...
	public static function ConvertToRFC1341($data, $restrictmore = false) {
		...
		for ($x = 0; $x < $y; $x ++) {
			...
			if ($currchr == 9 || $currchr == 32 || ($currchr >= 37 && $currchr <= 45) || ($currchr >= 47 && $currchr <= 60) || $currchr == 62 || $currchr == 63 || ($currchr >= 65 && $currchr <= 90) || $currchr == 95 || ($currchr >= 97 && $currchr <= 122)) {
				if (!$restrictmore) {
					$data2 .= $data[$x];
				} else if (($currchr >= 48 && $currchr <= 57) || ($currchr >= 65 && $currchr <= 90) || ($currchr >= 97 && $currchr <= 122)) {
		...
		foreach ($data2 as $currline) {
			...
			if ($x2 < $y2) {
				$result .= substr($currline, $x2, $y2 - $x2);
			}
			$result .= "\r\n";
		}
	...
	public static function ConvertToRFC1342($data, $lang = "UTF-8", $encodeb64 = true) {
		...
		if (strlen($lang) > 75 - 6 - 1 - 4) {
			return $result;
		}

		$lang = strtoupper($lang);
		if ($lang != "ISO-8859-1" && $lang != "US-ASCII") {
			$encodeb64 = true;
		}
	...
	public static function MakeValidEmailAddress($email, $options = array()) {
			...
			switch ($state) {
				...
				case "ipaddr":
				{
					if ($lastchr == "[" || $lastchr == "{" || $lastchr == "@") {
						...
						$domain .= ":";
					} else if (preg_match('/[A-Za-z0-9]/', $lastchr)) {
						$domain .= $lastchr;
					}
		...
		if (!$y) {
			return array(
				"success"   => false,
				"error"     => self::SMTP_Translate("Missing local part of e-mail address."),
				"errorcode" => "missing_local_part",
				"info"      => $email
			);
		}
		if (!$y2) {
			return array(
				"success"   => false,
				"error"     => self::SMTP_Translate("Missing domain part of e-mail address."),
				"errorcode" => "missing_domain_part",
				"info"      => $email
			);
		}
		if ($y > 64 || $y2 > 253 || $y + $y2 + 1 > 253) {
			return array(
				"success"   => false,
				"error"     => self::SMTP_Translate("E-mail address is too long."),
		...
		if (substr($domain, 0, 1) == "[" && substr($domain, - 1) == "]") {
		...
		} else if (isset($options["usedns"]) && $options["usedns"] === false) {
			$result = array(
				"success" => true,
				"email"   => $email,
				"lookup"  => false,
		...
		} else if ((!isset($options["usednsttlcache"]) || $options["usednsttlcache"] === true) && isset(self::$dnsttlcache[$domain]) && self::$dnsttlcache[$domain] >= time()) {
			$result = array(
				"success" => true,
				"email"   => $email,
				"lookup"  => false,
			...
			$result = self::GetDNSRecord($domain, array(
				"MX",
				"A"
			), (isset($options["nameservers"]) ? $options["nameservers"] : array(
				"8.8.8.8",
				"8.8.4.4"
			)), (!isset($options["usednsttlcache"]) || $options["usednsttlcache"] === true));
			if ($result["success"]) {
				$result = array(
					"success" => true,
					"email"   => $email,
	...
	public static function UpdateDNSTTLCache() {
		...
		foreach (self::$dnsttlcache as $domain => $ts2) {
			if ($ts2 > $ts) {
				unset(self::$dnsttlcache[$domain]);
			}
		}
	}

	public static function GetDNSRecord(
		$domain, $types = array("MX", "A"), $nameservers = array(
		"8.8.8.8",
		"8.8.4.4"
	), $cache = true
	) {
		...
		try {
			...
			return array(
				"success"   => false,
				"error"     => self::SMTP_Translate("Invalid domain name or missing DNS record."),
				"errorcode" => "invalid_domain_or_missing_record",
				"info"      => $domain
			);
		} catch (Exception $e) {
			return array(
				"success"   => false,
				"error"     => self::SMTP_Translate("Invalid domain name.  Internal exception occurred."),
				"errorcode" => "dns_library_exception",
				"info"      => self::SMTP_Translate("%s (%s).", $e->getMessage(), $domain)
			);
		}
	}
			...
			while ($data != "" && $state != "") {
				...
				switch ($state) {
					...
					case "local":
					{
						...
						} else if ($lastchr == " " || $lastchr == "," || $lastchr == ";") {
							$state = "localstart";
						} else {
							$data = substr($data, 0, - 1);
						}
	...
	public static function EmailAddressesToEmailHeaders($emailaddrs, $headername, $multiple = true, $removenames = false, $options = array()) {
		...
		for ($x = 0; $x < $y && ($multiple || $result == ""); $x ++) {
			...
			if ($name != "" && !UTF8::IsASCII($name)) {
				$name = self::ConvertToRFC1342($name) . " ";
			} else if ($name != "") {
				$name = '"' . $name . '" ';
			}
			if ($result != "") {
	...
	public static function GetUserAgent($type) {
		if ($type == "Thunderbird") {
			return "User-Agent: Mozilla/5.0 (X11; Linux x86_64; rv:24.0) Gecko/20100101 Thunderbird/24.0\r\n";
		} else if ($type == "Thunderbird2") {
			return "X-Mailer: Thunderbird 2.0.0.16 (Windows/20080708)\r\n";
		} else if ($type == "OutlookExpress") {
			return "X-Mailer: Microsoft Outlook Express 6.00.2900.3198\r\nX-MimeOLE: Produced By Microsoft MimeOLE V6.00.2900.3198\r\n";
		} else if ($type == "Exchange") {
			return "X-Mailer: Produced By Microsoft Exchange V6.0.6619.12\r\n";
		} else if ($type == "OfficeOutlook") {
			return "X-Mailer: Microsoft Office Outlook 12.0\r\n";
		}
		...
		while (strpos($state["data"], "\n") === false) {
			...
			if ($data2 === false || $data2 === "") {
				if ($state["async"]) {
					return array(
						"success"   => false,
						"error"     => self::SMTP_Translate("Non-blocking read returned no data."),
						"errorcode" => "no_data"
					);
				} else if ($data2 === false) {
					return array(
						"success"   => false,
						"error"     => self::SMTP_Translate("Underlying stream encountered a read error."),
			...
			if ($data2 === false || strpos($data2, "\n") === false) {
				if (feof($state["fp"])) {
					return array(
						"success"   => false,
						"error"     => self::SMTP_Translate("Remote peer disconnected."),
				...
				if (self::StreamTimedOut($state["fp"])) {
					return array(
						"success"   => false,
						"error"     => self::SMTP_Translate("Underlying stream timed out."),
			...
			if ($state["timeout"] !== false && self::GetTimeLeft($state["startts"], $state["timeout"]) == 0) {
				return array(
					"success"   => false,
					"error"     => self::SMTP_Translate("SMTP timeout exceeded."),
	...
	private static function ProcessState__WriteData(&$state) {
		if ($state["data"] !== "") {
			...
			if ($result === false || feof($state["fp"])) {
				return array(
					"success"   => false,
					"error"     => self::SMTP_Translate("A fwrite() failure occurred.  Most likely cause:  Connection failure."),
			...
			if (isset($state["options"]["sendratelimit"])) {
				...
				if (microtime(true) < $state["waituntil"]) {
					return array(
						"success"   => false,
						"error"     => self::SMTP_Translate("Rate limit for non-blocking connection has not been reached."),
						"errorcode" => "no_data"
					);
				}
			}
		...
		while ($state["state"] !== "done") {
			switch ($state["state"]) {
				case "connecting":
				{
					if (function_exists("stream_select") && $state["async"]) {
						...
						if ($result === false) {
							return self::CleanupErrorState($state, array(
								"success"   => false,
								"error"     => self::SMTP_Translate("A stream_select() failure occurred.  Most likely cause:  Connection failure."),
						...
						if (!count($writefp)) {
							return array(
								"success"   => false,
								"error"     => self::SMTP_Translate("Connection not established yet."),
								"errorcode" => "no_data"
							);
						}
					}
					...
					if (function_exists("stream_context_get_options")) {
						...
						if ($state["secure"] && isset($state["options"]["sslopts"]) && is_array($state["options"]["sslopts"]) && isset($contextopts["ssl"]["peer_certificate"])) {
							if (isset($state["options"]["debug_callback"]) && is_callable($state["options"]["debug_callback"])) {
								call_user_func_array($state["options"]["debug_callback"], array(
									"peercert",
									@openssl_x509_parse($contextopts["ssl"]["peer_certificate"]),
									&$state["options"]["debug_callback_opts"]
								));
							}
						}
				...
				case "get_response":
				{
					...
					if (strlen($currline) >= 4) {
						...
						if (substr($currline, 3, 1) == " ") {
							if ($state["expectedcode"] > 0 && $state["code"] !== $state["expectedcode"]) {
								return self::CleanupErrorState($state, array(
									"success"   => false,
									"error"     => $state["expectederror"],
									"errorcode" => "invalid_response",
									"info"      => $state["code"] . " " . $state["response"]
								));
							}
				...
				case "esmtp_extensions":
				{
					...
					$y        = count($smtpdata);
					for ($x = 1; $x < $y; $x ++) {
						if (strtoupper(substr($smtpdata[$x], 0, 4)) == "AUTH" && ($smtpdata[$x][4] == ' ' || $smtpdata[$x][4] == '=')) {
							$auth = strtoupper(substr($smtpdata[$x], 5));
						}
						if (strtoupper(substr($smtpdata[$x], 0, 4)) == "SIZE" && ($smtpdata[$x][4] == ' ' || $smtpdata[$x][4] == '=')) {
							$state["size_supported"] = (int) substr($smtpdata[$x], 5);
						}
					}
	...
	public static function SendSMTPEmail($toaddr, $fromaddr, $message, $options = array()) {
		...
		if (!function_exists("stream_socket_client") && !function_exists("fsockopen")) {
			return array(
				"success"   => false,
				"error"     => self::SMTP_Translate("The functions 'stream_socket_client' and 'fsockopen' do not exist."),
		...
		$tempfromnames = array();
		$tempfromaddrs = array();
		if (!self::EmailAddressesToNamesAndEmail($temptonames, $temptoaddrs, $toaddr, true, $options)) {
			return array(
				"success"   => false,
				"error"     => self::SMTP_Translate("Invalid 'To' e-mail address(es)."),
				"errorcode" => "invalid_to_address",
				"info"      => $toaddr
			);
		}
		if (!self::EmailAddressesToNamesAndEmail($tempfromnames, $tempfromaddrs, $fromaddr, true, $options)) {
			return array(
				"success"   => false,
				"error"     => self::SMTP_Translate("Invalid 'From' e-mail address."),
		...
		if (isset($options["fp"]) && is_resource($options["fp"])) {
			...
			if (!function_exists("stream_socket_client")) {
				$fp = @fsockopen(($secure ? "tls://" : "") . $server, $port, $errornum, $errorstr, $options["connecttimeout"]);
			} else {
			...
			if ($fp === false) {
				return array(
					"success"   => false,
					"error"     => self::SMTP_Translate("Unable to establish a SMTP connection to '%s'.", ($secure ? "tls://" : "") . $server . ":" . $port),
	...
	public static function SMTP_HTMLTagFilter($stack, &$content, $open, $tagname, &$attrs, $options) {
		...
		if ($tagname === "a" && (!isset($attrs["href"]) || trim($attrs["href"]) === "")) {
			return array("keep_tag" => false, "keep_interior" => false);
		}
		if ($tagname === "/a" && $stack[0]["keep_interior"]) {
			if ($stack[0]["attrs"]["href"] === trim($content)) {
				$content = " [ " . trim($content) . " ] ";
			} else if (trim($content) !== "") {
				$content = " " . trim($content) . " (" . trim($stack[0]["attrs"]["href"]) . ") ";
			}
		}
		if ($tagname === "img") {
			if (!isset($attrs["src"])) {
			...
			if (isset($attrs["alt"]) && trim($attrs["alt"]) !== "" && trim($attrs["alt"]) !== $attrs["src"]) {
				$content .= trim($attrs["alt"]) . "\n\n";
			}
		}

		if ($tagname === "table" || $tagname === "blockquote" || $tagname === "ul") {
			self::$depths[] = $tagname;
		}
		if ($tagname === "ol") {
			self::$depths[] = 1;
		...
		if (trim($content) !== "") {
			...
			if ($tagname === "/ul" || $tagname === "/ol" || $tagname === "/table" || $tagname === "/blockquote") {
				...
				if ($tagname === "/ul" || $tagname === "/ol") {
					$prefix .= "\xFF\xFF" . (count(self::$depths) > 1 ? "\xFF\xFF" : "");
				}
				if ($tagname === "/blockquote") {
				...
				foreach ($lines as $num => $line) {
					if (trim($line) !== "") {
						if ($line{0} !== "\xFF" && (($tagname === "/ul" && $line{0} !== "-") || ($tagname === "/ol" && !(int) $line{0}))) {
							$prefix2 = "\xFF\xFF";
						} else {
							$prefix2 = "";
						}
		...
		if ($tagname === "/table" || $tagname === "/blockquote" || $tagname === "/ul" || $tagname === "/ol") {
			array_pop(self::$depths);
		}
	...
	public static function SendEmailAsync__Handler($mode, &$data, $key, &$info) {
		switch ($mode) {
			...
			case "readfps":
			{
				if ($info["state"] !== false && self::WantRead($info["state"])) {
					$data[$key] = $info["state"]["fp"];
				}
			...
			case "writefps":
			{
				if ($info["state"] !== false && self::WantWrite($info["state"])) {
					$data[$key] = $info["state"]["fp"];
				}
	...
	public static function SendEmail($fromaddr, $toaddr, $subject, $options = array()) {
		...
		if ($messagefromaddr == "" && $replytoaddr == "") {
			return array(
				"success"   => false,
				"error"     => self::SMTP_Translate("From address is invalid."),
				"errorcode" => "invalid_from_address",
				"info"      => $fromaddr
			);
		}
		...
		if ($htmlmessage == "" && !count($attachments)) {
			...
			} else if ($textmessage != "" && $htmlmessage != "") {
		...
		if (isset($options["returnresults"]) && $options["returnresults"]) {
			...
			if (!$result) {
				return array(
					"success"   => false,
					"error"     => self::SMTP_Translate("PHP mail() call failed."),
					"errorcode" => "mail_call_failed"
				);
			}
