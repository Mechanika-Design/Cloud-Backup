...
class AmazonCloudDrive {
	...
	public function InteractiveLogin__HandleRequest(&$state) {
		if (substr($state["url"], 0, strlen($this->data["returnurl"])) === $this->data["returnurl"]) {
			...
			if (isset($url["queryvars"]["error"])) {
				echo self::ACD_Translate("Unfortunately, an error occurred:  %s (%s)\n\nDid you deny/cancel the consent?\n\n", $url["queryvars"]["error_description"][0], $url["queryvars"]["error"][0]);
			} else {
				echo self::ACD_Translate("Retrieving refresh token from Amazon...\n");

				$result = $this->UpdateRefreshToken($url["queryvars"]["code"][0]);
				if (!$result["success"]) {
					echo self::ACD_Translate("Unfortunately, an error occurred while attempting to retrieve tokens and endpoint URLs:  %s (%s)\n\n", $result["error"], $result["errorcode"]);
				} else {
					echo self::ACD_Translate("Refresh token, initial bearer token, and endpoint URLs successfully retrieved!\n\n");
				}
			}
	...
	public function InteractiveLogin() {
		...
		$web        = new WebBrowser(array(
			"httpopts" => array(
				"pre_retrievewebpage_callback" => array(
					$this,
					"InteractiveLogin__HandleRequest"
				)
			)
		));
		...
		do {
			...
			if (!$result2["success"]) {
				if ($this->data["refreshtoken"] === false) {
					return $result2;
				}
				...
				return array(
					"success"   => false,
					"error"     => self::ACD_Translate("Expected a 200 response from Amazon.  Received '%s'.", $result["response"]["line"]),
					"errorcode" => "unexpected_amazon_response",
					"info"      => $result2
				);
			} else {
				...
				if ($text) {
					echo trim(preg_replace('/\s+/', " ", $text->plaintext)) . "\n\n";
				}

				$forms = $web->ExtractForms($result2["url"], $body);

				foreach ($forms as $num => $form) {
					if ($form->info["action"] === "https://www.amazon.com/ap/get" && $form->info["name"] === "ue_backdetect") {
						unset($forms[$num]);
					}
				}
				...
				if ($result === false) {
					return array(
						"success"   => false,
						"error"     => self::ACD_Translate("Expected at least one form to exist.  Received none."),
						"errorcode" => "invalid_amazon_response",
						"info"      => $result2
					);
				}
			}
	...
	public function UpdateEndpoints() {
		if ($this->data["bearertoken"] === false) {
			return array(
				"success"   => false,
				"error"     => self::ACD_Translate("Unable to update the endpoints.  Bearer/Authorization token is missing.  Has the configuration been run?"),
				"errorcode" => "no_bearer_token"
			);
		}
	...
	public function UpdateBearerToken() {
		if ($this->data["refreshtoken"] === false) {
			return array(
				"success"   => false,
				"error"     => self::ACD_Translate("Unable to update the bearer token.  Refresh token is missing.  Has the configuration been run?"),
	...
	public function GetFolderList($folderid) {
		if ($folderid === false) {
			return array(
				"success"   => false,
				"error"     => self::ACD_Translate("Invalid folder ID."),
		...
		do {
			...
			foreach ($result["body"]["data"] as $item) {
				if ($item["kind"] === "FOLDER") {
					$folders[isset($item["name"]) ? $item["name"] : ""] = $item;
				} else if ($item["kind"] === "FILE") {
					$files[isset($item["name"]) ? $item["name"] : ""] = $item;
				}
			}
	...
	public function GetObjectByID($id) {
		if ($id === false) {
			return array(
				"success"   => false,
				"error"     => self::ACD_Translate("Invalid object ID."),
	...
	public function MoveObject($srcparentid, $srcid, $destid) {
		if ($srcparentid === false) {
			return array(
				"success"   => false,
				"error"     => self::ACD_Translate("Invalid parent folder ID."),
				"errorcode" => "invalid_parent_folder_id"
			);
		}
		if ($srcid === false || $destid === false) {
			return array(
	...
	public function UploadFile($folderid, $destfilename, $data, $srcfilename, $fileid = false, $callback = false, $callbackopts = false) {
		...
		if ($fileid === false) {
			...
			if ($result["info"] !== false) {
				if ($result["info"]["kind"] !== "FILE") {
					return array(
						"success"   => false,
						"error"     => self::ACD_Translate("Parent folder already contains an object named '%s' that is not a file.", $destfilename),
		...
		if ($srcfilename !== false) {
			$fileinfo["datafile"] = $srcfilename;
		} else {
			$fileinfo["data"] = $data;
		}
	...
	public function DownloadFile($destfileorfp, $fileid, $callback = false) {
		if ($fileid === false) {
			return array(
				"success"   => false,
				"error"     => self::ACD_Translate("Invalid file ID."),
		...
		if ($destfileorfp === false) {
			...
			if ($fp === false) {
				return array(
					"success"   => false,
					"error"     => self::ACD_Translate("Invalid destination filename or handle."),
		...
		if ($destfileorfp !== false && !is_resource($destfileorfp)) {
			fclose($fp);
		}
	...
	private function RunAPI($method, $url, $options = array(), $expected = 200, $decodebody = true) {
		...
		if ($result["response"]["code"] != $expected) {
			return array(
				"success"   => false,
				"error"     => self::ACD_Translate("Expected a %d response from Amazon.  Received '%s'.", $expected, $result["response"]["line"]),
				"errorcode" => "unexpected_amazon_response",
				"info"      => $result
			);
		}
