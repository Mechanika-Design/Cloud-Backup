...
class CB_Service_local {
	...
	public function BuildRemotePath() {
		...
		if (!is_dir($path)) {
			...
			if ($result === false) {
				return array("success"   => false,
				             "error"     => self::LTranslate("Unable to locate or create base path '%s' or path exists but is not a directory.", $path),
		...
		if ($path2 === false) {
			return array("success"   => false,
			             "error"     => self::LTranslate("Unable to get the real path for '%s'.", $path),
			             "errorcode" => "invalid_path"
			);
		}
		...
		foreach ($result["folders"] as $name) {
			if ($name === "TEMP") {
				...
				if (is_numeric($name) && preg_match('/^\d+$/', $name)) {
					$this->incrementals[(int) $name] = $name;
				}
			}
		}
	...
	public function StartBackup() {
		...
		if ($result === false) {
			return array("success"   => false,
			             "error"     => self::LTranslate("Unable to create base path '%s'.", $path),
			             "errorcode" => "invalid_path"
			);
		}
	...
	public function UploadBlock($blocknum, $part, $data) {
		...
		if ($result === false) {
			return array("success"   => false,
			             "error"     => self::LTranslate("Unable to write data to '%s'.", $filename),
	...
	public function FinishBackup($backupid) {
		...
		if ($result === false) {
			return array("success"   => false,
			             "error"     => self::LTranslate("Unable to rename 'TEMP' to '%s'.", (string) count($incrementals)),
	...
	public function DownloadBlock($info) {
		...
		if ($data === false) {
			return array("success"   => false,
			             "error"     => self::LTranslate("Unable to read data from '%s'.", $filename),
			             "errorcode" => "invalid_filename"
			);
		}
	...
	public function MoveBlockIntoMergeBackup($parts) {
		foreach ($parts as $part) {
			...
			if ($result === false) {
				return array("success"   => false,
				             "error"     => self::LTranslate("Unable to move '%s' to '%s'.", $srcfilename, $destfilename),
				             "errorcode" => "rename_failed"
				);
			}
		}
	...
	private function GetFolderList($path) {
		...
		if ($dir === false) {
			return array("success"   => false,
			             "error"     => self::LTranslate("Unable to open the directory '%s' for reading.", $path),
			             "errorcode" => "opendir"
			);
		}
	...
	private function DeleteFolder($path) {
		if (is_link($path)) {
			...
			if ($result === false) {
				return array("success"   => false,
				             "error"     => self::LTranslate("Unable to remove symbolic link '%s'.", $path),
				             "errorcode" => "unlink"
				);
			}
		} else {
			...
			while (($file = readdir($dir)) !== false) {
				if ($file !== "." && $file !== "..") {
					...
					if (is_dir($filename)) {
						...
						if ($result === false) {
							return array("success"   => false,
							             "error"     => self::LTranslate("Unable to remove '%s'.", $filename),
							             "errorcode" => "unlink"
							);
						}
					}
			...
			if ($result === false) {
				return array("success"   => false,
				             "error"     => self::LTranslate("Unable to remove directory '%s'.", $path),
				             "errorcode" => "rmdir"
				);
			}
		}
