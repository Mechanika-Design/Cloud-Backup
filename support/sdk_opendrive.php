...
class OpenDrive {
	...
	public function GetFolderList($folderid = "0") {
		if ($this->sessionid === false) {
			return array(
				"success"   => false,
				"error"     => self::OD_Translate("Not logged into OpenDrive."),
				"errorcode" => "no_login"
			);
		}
	...
	public function TrashFolder($id, $remove = false) {
		...
		if ($remove) {
			$result = $this->RemoveTrashedFolder($id);
		}
	...
	public function UploadFile($folderid, $filename, $dataorfp, $size = false, $fileid = false, $callback = false) {
		...
		if (is_resource($dataorfp)) {
			...
			if ($size > strlen($dataorfp)) {
				$size = strlen($dataorfp);
			}
		...
		if ($fileid === false) {
			...
			if ($result["success"]) {
				...
				if ($result2["info"] === false || !isset($result2["info"]["FileId"])) {
					return $result;
				}
	...
	public function DownloadFile($destfileorfp, $fileid, $callback = false) {
		...
		if ($destfileorfp === false) {
			...
			if ($fp === false) {
				return array(
					"success"   => false,
					"error"     => self::OD_Translate("Invalid destination filename or handle."),
	...
	public function TrashFile($id, $remove = false) {
		...
		if ($remove) {
			$result = $this->RemoveTrashedFile($id);
		}
	...
	private function RunAPI($method, $apipath, $options = array(), $expected = 200, $encodejson = true, $decodebody = true) {
		...
		if ($result["response"]["code"] != $expected) {
			return array(
				"success"   => false,
				"error"     => self::OD_Translate("Expected a %d response from OpenDrive.  Received '%s'.", $expected, $result["response"]["line"]),
				"errorcode" => "unexpected_opendrive_response",
				"info"      => $result
			);
		}
