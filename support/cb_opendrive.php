...
if (!class_exists("OpenDrive", false)) {
	require_once str_replace("\\", "/", dirname(__FILE__)) . "/sdk_opendrive.php";
}
...
class CB_Service_opendrive {
			...
			foreach ($result["body"]["Folders"] as $info2) {
				if ($info2["Name"] === "TEMP") {
					...
					if (is_numeric($info2["Name"]) && preg_match('/^\d+$/', $info2["Name"])) {
						$this->incrementals[(int) $info2["Name"]] = $info2["FolderID"];
					}
				}
			}
		}
	...
	public function StartMergeDown() {
		...
		if ($result["info"] !== false) {
			$this->remotemergefolderid = $result["info"]["FolderID"];
		} else {
