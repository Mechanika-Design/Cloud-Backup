...
class CB_ServiceHelper {
	...
	public function InitService($config) {
		...
		if (isset($config["service_info"]["options"])) {
			$this->service->Init($config["service_info"]["options"], $this);
		}
	...
	public function ApplyPhoto(&$data) {
		if ($this->img === false) {
			...
			if (!$result["success"]) {
				CB_DisplayError("A network error occurred while retrieving a photo from third-party service.", $result);
			} else if ($result["response"]["code"] != 200) {
				CB_DisplayError("Expected a 200 response from third-party photo service.  Received '" . $result["response"]["line"] . "'.", $result);
			}
	...
	public function UnapplyPhoto(&$data) {
		...
		$pos = strpos($data, $str);
		if ($pos === false) {
			CB_DisplayError("Magic token is missing in the photo data.");
		}
		$pos += strlen($str);
