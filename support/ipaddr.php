...
class IPAddr {
	...
	static function GetRemoteIP($proxies = array()) {
		...
		if (isset($proxies[$ipaddr["ipv6"]]) || ($ipaddr["ipv4"] != "" && isset($proxies[$ipaddr["ipv4"]]))) {
			...
			do {
				...
				if (isset($proxies[$ipaddr["ipv6"]])) {
					$header = $proxies[$ipaddr["ipv6"]];
				} else {
					$header = $proxies[$ipaddr["ipv4"]];
				}
	...
	static function IsMatch($pattern, $ipaddr) {
		if (is_string($ipaddr)) {
			$ipaddr = self::NormalizeIP($ipaddr);
		}
		...
		if (strpos($pattern, ":") !== false) {
			...
			if (count($pattern) != 8 || count($ipaddr) != 8) {
				return false;
			}
			...
			if (count($pattern) != 4 || count($ipaddr) != 4) {
				return false;
			}
			foreach ($pattern as $num => $segment) {
				$found  = false;
				...
				foreach ($pieces as $piece) {
					...
					if ($piece == "*") {
						...
						if ($ipaddr[$num] >= $range[0] && $ipaddr[$num] <= $range[1]) {
							$found = true;
						}
					} else if ($piece == $ipaddr[$num]) {
						$found = true;
